...
local function get_display_lines(text)
  ...
  local extra_indent =
    math.min(unpack(vim.tbl_map(function(line) return #(string.match(line, "^%s*") or "") end, lines)))
...
local function panel_uri_from_doc_uri(doc_uri) return doc_uri:gsub("^file://", panel_uri_prefix) end

---@return string doc_uri
local function panel_uri_to_doc_uri(panel_uri) return panel_uri:gsub("^" .. panel_uri_prefix, "file://") end

---@param bufname string
local function is_panel_uri(bufname) return bufname:sub(1, #panel_uri_prefix) == panel_uri_prefix end
...
function panel:add_entry(item)
  if panel.state.entries[item.solutionId] then return self end
...
function panel:get_entry(dir)
  ...
  if type(dir) == "string" then return self.state.entries[dir] end
  ...
  if not extmarks[1] then return nil end
...
function panel:jump(dir)
  ...
  if linenr then vim.api.nvim_win_set_cursor(self.winid, { linenr, 0 }) end
end
...
function panel:close()
  if self.bufnr and vim.api.nvim_win_is_valid(self.bufnr) then self:unlock():clear():lock() end

  if self.winid and vim.api.nvim_win_is_valid(self.winid) then vim.api.nvim_win_close(self.winid, true) end
end
...
function panel:ensure_winid()
  ...
  vim.api.nvim_create_autocmd("WinClosed", {
    ...
    callback = function()
      ...
      if should_jump_to_prev_win then vim.cmd "wincmd p" end
    end,
...
function panel:refresh()
  ...
  if not c.buf_is_attached(0) then return end

  if self.panelId then api.unregister_panel_handlers(self.panelId) end
...
function mod.accept() panel:accept() end

function mod.jump_prev() panel:jump(-1) end

function mod.jump_next() panel:jump(1) end

function mod.refresh()
  vim.api.nvim_buf_call(vim.uri_to_bufnr(panel_uri_to_doc_uri(panel.panel_uri)), function() panel:refresh() end)
end
...
function mod.setup()
  ...
  if panel.setup_done then return end
...
function mod.teardown()
  ...
  if not panel.setup_done then return end

  if panel.keymap.open then vim.keymap.del("i", panel.keymap.open) end
